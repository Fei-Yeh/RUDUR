function [Aest,Fest,fObj,fWLS,fPrior,fRegF,finalIter] = rudur(Y,M,alpha,beta,gamma,mu,Q,S)
...
%   (optional) Q : Vector with weight for each image
%   (optional) S : Vector with weight for Tychonof regularization
% @return : 
...
[fObj(iter),fWLS(iter),fPrior(iter),fRegF(iter)] = compute_fObj(Y,Aest,Fest,W2,D2,alpha,beta,mu,Q,S);


%% Start optimization
for iter=2:maxIter
    if varMin==0    % Fix F and minimize according to A
        [Aest,Fest]=optimA_ConjGrad(Y,Aest,Fest,W2,D2,alpha,mu,Q);
    else            % Fix A and minimize according to F
        [Aest,Fest]=optimF_ConjGrad(Y,Aest,Fest,W2,D2,beta,Q,S);
    end
